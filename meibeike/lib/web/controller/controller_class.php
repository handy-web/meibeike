...
//     	if(!$this->theme)
//     	{
    		$client    = $this->module->clientType; //获取客户端类型
   
    		$themeList = isset($this->module->config['theme']) ? $this->module->config['theme'] : null;
    	
    		if($themeList && isset($themeList[$client]))
    		{
    			$this->theme = $themeList[$client];
...
//        return isset($this->module->config['viewPath']) ? $this->module->config['viewPath'] : $this->defaultViewPath;
     ...
     * @brief 网站版权信息
     */
    public function showMyVerId() {
        echo "<script>alert('授权信息：内核-Iwebshop，由济南爱程网络科技有限公司，依据合同201410180001授权huliancheng.com为第三方开发商用软件。')</script>";
        $this->redirect("/site/index",false);
    }
