...
//调用存储过程写入预约信息
    ...
    public static function write_appoint3($v_appointment_no,$v_purchase_time,$v_user_type,$v_mei_id,$v_vendor_id, $v_pic_url,$from='wap',$ip) {
        //PROCEDURE proc_appointment_add(IN v_appointment_no varchar(32),IN v_purchase_time int, IN v_user_type int, IN v_mei_id int,IN v_vendor_id varchar(32),IN v_pic_url varchar(128) , OUT v_result int)    
//         var_dump("call proc_appointment_add('$v_appointment_no','$v_purchase_time','$v_user_type','$v_mei_id','$v_vendor_id','$v_pic_url',\"$from\",'$ip',@ret)");
//         exit;
    	try {
            $ret = array();
            $db = IDBFactory::getDB();
            $db->query("call proc_appointment_add('$v_appointment_no','$v_purchase_time','$v_user_type','$v_mei_id','$v_vendor_id','$v_pic_url',\"$from\",'$ip',@ret)");             
            //$ret = $db->query("select @ret;");
            ...
            $ret = $db->query("select @v_appointment_no v_appointment_no,@ret;");
           // var_dump($ret);
            $userArray = array(
                'appointment_no' => $ret[0]["v_appointment_no"],
                'ret' => $ret[0]["@ret"]
            );
            return $userArray;
...
//调用存储过程发送邮件
    ...
    public static function add_lottery($v_uid,$v_meiid,$v_prize_id = '',$v_activity_type,$v_time,$v_ip){
        try {
            $ret = array();
            $db = IDBFactory::getDB();
            $db->query("call proc_add_lottery('$v_uid','$v_meiid','$v_prize_id','$v_activity_type','$v_time','$v_ip',@ret)");
            // 返回值 0:已经抽过奖  1: 保存成功
            //do {
            if ($result = $db->linkRes->store_result()) {
                while ($row = $result->fetch_row()) {
    ...
    public static function appointment_query($v_user_id,$v_purchase_time){
        try {
            $ret = array();
            $db = IDBFactory::getDB();
            $db->query("call proc_appointment_result('$v_user_id','$v_purchase_time',@ret)");
            // 返回值 0:已经预约  1: 没有预约
            //do {
            if ($result = $db->linkRes->store_result()) {
                while ($row = $result->fetch_row()) {
    ...
    public static function check_lottery($v_uid,$v_meiid,$v_activity_type){
        try {
            $ret = array();
            $db = IDBFactory::getDB();
            $db->query("call proc_check_lottery('$v_uid','$v_meiid','$v_activity_type',@ret)");
            // 返回值 0:已经预约  1: 没有预约
            //do {
            if ($result = $db->linkRes->store_result()) {
                while ($row = $result->fetch_row()) {
