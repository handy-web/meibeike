...
class Order_Class {
    ...
    public static function createOrderNum() {
        //return 'B' . date('ymdHis') . rand(100000, 999999);
        return str_replace("0.","",str_replace(" ","",microtime())).sprintf("%06d",ISafe::get("user_id")). rand(1000, 9999);
    }

    /**
     * @brief 产生服务单ID
     */
    public static function createServiceNum() {
        return 'S' . date('YmdHis') . rand(100000, 999999);
    }
    ...
    public static function updateOrderStatus($orderNo, $admin_id = '', $note = '') {
        ...
        if ($orderRow['pay_status'] == 1) {
            return $orderRow['id'];
        } else if ($orderRow['pay_status'] == 0 || $orderRow['pay_status'] == -1) {
            $dataArray = array(
    ...
    /**
     * @brief 获取服务订单基本数据资料
     * @param $order_id int 服务订单的id
     * @return array()
     */
    public function getOrderServiceShow($order_id) {
        $data = array();
   ...
   //判断变量是数组还是单个变量
    public static function getWhere2($id) {
        if (is_array($id)) {
            $id = join(',', $id);
    ...
    //获取服务单状态文字
    public static function getServiceStatusText($status) {
        switch ($status) {
            case 1: {
                    return '提交服务单';
                }
     ...
     * @param string $sendor 操作者所属 admin,seller
     */
    public static function sendDeliveryGoodsService($order_id, $sendor_id, $sendor = 'admin') {
        ...
        $tb_order->update('order_id=' . $order_id);
        ...
        $tb_order_log->setData(array(
            ...
            'action' => '服务订单发货',
            'result' => '成功',
            'note' => '服务订单【' . $service_no . '】由【' . $sendorSort . '】' . $sendorName . '发货',
            'addtime' => date('Y-m-d H:i:s')
        ));
        $sendResult = $tb_order_log->add();
...
public static function getprocityname($id)
	{
		$goodsDB  = new IModel('areas');
		$goodsRow = $goodsDB->getObj('area_id = '.$id,'area_name');
		return $goodsRow ?$goodsRow['area_name']:"";
	
	}

	public static function getoperatorname($id)
	{
		$goodsDB  = new IModel('fahuo_log');
		$goodsRow = $goodsDB->getObj('order_id = '.$id,'user_id');
		return $goodsRow ?$goodsRow['user_id']:"";
	
	}
	public static function getoperatortime($id)
	{
		$goodsDB  = new IModel('fahuo_log');
		$goodsRow = $goodsDB->getObj('order_id = '.$id,'createtime');
		return $goodsRow ?$goodsRow['createtime']:"";
	
	}
	//获取订单配送状态
    public static function getOrderGoodsNameText($orderRow) {
        
        return json_decode($orderRow)->name;
