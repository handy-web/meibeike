...
class CountSum
{
	...
	public function goodsCount($buyInfo)
	{
    	...
    	if(isset($buyInfo['product']['id']) && $buyInfo['product']['id'])
    	{
    		...
    		foreach($productList as $key => $val)
    		{
				...
				$productList[$key]['reduce'] = sprintf('%.2f',$productList[$key]['reduce']);
    			...
    			$productList[$key]['sum']    = sprintf('%.2f',$productList[$key]['sum']);
    			//全局统计
    	...
    	$this->final_sum = sprintf('%.2f',$this->final_sum);
    	return array(
	 ...
	 * @param IQuery 结果集对象
	 */
    public static function getSellerGoodsFeeQuery($seller_id = '',$start_time = '',$end_time = '')
    {
    	$where  = "og.is_send = 1 and o.pay_type != 0";
