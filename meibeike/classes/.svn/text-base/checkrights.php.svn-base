	...
	public static function getUser()
	{
		$user = array(
			...
			'email' => ISafe::get('email'),
              'mobile' => ISafe::get('mobile'),
             'type' =>ISafe::get('type'),
			'username' => ISafe::get('username'),
			'head_ico' => ISafe::get('head_ico'),
			'user_pwd' => ISafe::get('user_pwd'),
            'active' => ISafe::get('active'),
			
                    
		);
                
                //var_dump($user);
		if ($user['mobile']!='' &&  self::isValidUser($user['mobile'],$user['user_pwd']))
		{
			return $user;
		}
		else if ($user['email']!='' &&  self::isValidUser($user['email'],$user['user_pwd']))
		{
			return $user;
		}
		else if(self::isValidUser($user['username'],$user['user_pwd']))
		{
			return $user;
		}else if($user['type'] == '7' || $user['type'] == '8' || $user['type'] == '9'){
			return $user;
		}
		else
		{
		...
		else
		{
			...
			{	
				...
				if(self::checkRight($roleRow['rights']) == false)
				{
					...
					echo "<script>alert('对不起，您无权限访问')</script>";
...
// 		setCookie("isValidUser_a",$login_info,$expire,"/");
// 		setCookie("isValidUser_b",$password,$expire,"/");
		...
		//先验证手机
		$where   = "u.mobile = '{$login_info}' and m.status = 1 and u.id = m.user_id and u.active=1";
		...
		{
			//验证email
			$where   = "email = '{$login_info}' and m.status = 1 and u.id = m.user_id and u.active=1";
			$userRow = $userObj->getObj($where);
		}
                
                if(empty($userRow))
		{//验证账号
			$where   = "u.username = '{$login_info}' and m.status = 1 and u.id = m.user_id and u.active=1";
			$userRow = $userObj->getObj($where);
		}
	...
	public static function isThirdValidUser($login_info,$password,$type)
	{
	    $login_info = IFilter::act($login_info);
	    $password   = IFilter::act($password);
	    $userObj = new IModel('user as u,third_user as t,member as m');
	    if(empty($type)){
	        return false;
	    }else{
	        $where   = "t.unionid = '{$login_info}' and t.uid = u.id and u.id = m.user_id and u.type = {$type}";
	        $userRow = $userObj->getObj($where,'u.*,m.*');
	    }
