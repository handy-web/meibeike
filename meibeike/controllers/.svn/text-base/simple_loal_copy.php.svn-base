...
class Simple extends IController {
    ...
    function verify_telphone() { 
       ...
       // var_dump($telphone);
        ...
        if(!empty($res) && !empty($res[0]['appointment_no'])){
            ...
            Db_Meicloud::sendMsg('美贝壳验证码：' . $vcode . '，该验证码十分钟内有效。预约云棒也同时奉送美贝壳账户，你可以以此手机号为MeiID登录美贝壳官网查询你的预约等。', "$telphone");
            $result['msg'] = $vcode;
            $result["code"] = 1;
            echo json_encode($result);
            exit;    
        ...
        if(!empty($res) && !empty($res[0]['appointment_no'])){
           ...
           // var_dump($userRow);exit;
            if (!empty($userRow)) {                     
                        //已经注册过,调用预约接口
                        $meiid = $userRow['meiid'];
                        $flag = $this->handAppoint($userRow['meiid'],1,$purchase_time,$userRow['id'],'wap');                      
                        if($flag!==false){
                            Db_Meicloud::sendMsg("恭喜您！预约成功，预约号为{$flag}。请记得6月20日10：00准时登录美贝壳官网抢购哟.同时您还可以使用该手机号码进行登录，登录密码是{$userRow['password']}，为保证帐户安全，登录后请尽快修改密码。", "$telphone");
                            $result['code'] = 1;
                    ...
                    if ($user_id) {
                            ...
                            $memberObj->add();
                            $flag = $this->handAppoint($meiid,$userType,$purchase_time,$user_id,'wap'); 
                                if($flag!==false){
    ...
    function handAppoint($meiid,$userType,$purchase_time,$id,$from){
        ...
        $temp_data = Db_Meicloud::write_appoint3($appointment_no,$purchase_time,$userType,$id,"","",$from);
