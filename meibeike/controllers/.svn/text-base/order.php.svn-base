...
public function order_chuhuo() {
        ...
        $where = '';
        if ($name != '') {$where .= " and (u.mobile like '%{$name}%' or u.email like '%{$name}%') ";}   //用户名搜索
        if ($order_no != '') {$where .= " and o.order_no like '%{$order_no}%' ";}		//订单号搜索
		if ($start_time != '') {$where .= " and o.create_time>='{$start_time}' ";}	  //开始时间 
		if ($end_time != '') {$where .= " and o.create_time<='{$end_time}' ";}   //结束时间
        ...
        $this->redirect("order_chuhuo");
    }
//收款管理
public function order_beihuo() {
        $data = array();
...
public function order_fahuo() {
        ...
        //向前台渲染
        $this->setRenderData($data);
        $this->redirect("order_fahuo");
    }

//发货管理修改发货状态
public function addfahuolog()
	{
		$oid = IFilter::act(IReq::get('oid'));
		$status = IFilter::act(IReq::get('status'));
        
		try{
		$fahuo_log = new IModel("fahuo_log");
		$fahuo_log->setData(array(status =>$status,order_id => $oid,user_id => ISafe::get('admin_name'),createtime => ITime::getDateTime()));
		$fahuo_log->add();
		
		$fahuo_log = new IModel("order");
		$fahuo_log->setData(array(fahuo_status =>$status));
		$fahuo_log->update("id=".$oid);
...
private function AppointmentUpt($status,$conmment,$a)
{
	try{
	$AppointmentUpt = new IModel("appointment");
	$AppointmentUpt->setData(array(status => $status,conmment => $conmment));
	$id=$AppointmentUpt->update("order_id=".$a);
	return $id;
	}
	catch(Exception $e) 
    {return $e; }
...
public function UptAppointment()
	{
		$SelectID = IFilter::act(IReq::get('SelectID'));
        $status = IFilter::act(IReq::get('status'));
		$conmment = IFilter::act(IReq::get('conmment'));
        ...
        {			
			$res=$this->AppointmentUpt($status,$conmment,$a);
		}
    ...
    public function Appoint_out()
	{
		$regtype = IFilter::act(IReq::get('regtype'));
        $vf = IFilter::act(IReq::get('vf'));
        $status = IFilter::act(IReq::get('status'));
		$usertype = IFilter::act(IReq::get('usertype'));
        $name = IFilter::act(IReq::get('name'));
        $order_no = IFilter::act(IReq::get('order_no'));
		$start_time = IFilter::act(IReq::get('start_time'));
        $end_time = IFilter::act(IReq::get('end_time'));
		...
		$where = '';
        if ($regtype != '') {$where .= ' and b.type = ' . $regtype;}   //注册类型 MeiID
        if ($usertype != '') {$where .= ' and a.user_type = ' . $usertype;}        //用户类型  普通，网盘
        if ($vf != '') {$where .= " and a.vf = {$vf}";}   //来源
		if ($status != '') {$where .= " and a.status = {$status}";}   //状态
		if ($name != '') {$where .= " and (b.mobile like '%{$name}%' or b.email like '%{$name}%') ";}   //用户名搜索
        if ($order_no != '') {$where .= " and a.appointment_no like '%{$order_no}%' ";}		//预约号搜索
		if ($start_time != '') {$where .= " and a.create_time>='{$start_time}' ";}	  //开始时间 
		if ($end_time != '') {$where .= " and a.create_time<='{$end_time}' ";}   //结束时间
		...
		$obj = new IQuery("user b");
        $obj->join = "join mbk_appointment a on a.mei_id = b.meiid";
        $obj->where = "1=1 and if_del=0".$where;
    	$obj->order = 'a.create_time desc';

        $obj->fields = "a.appointment_no,b.username,b.mobile,b.email,b.type,b.meiid,a.purchase_time,a.create_time,a.appointment_from,a.user_type"; 
        $res = $obj->find();
		foreach ($res as $v=>$a) 
		{
			if($a['user_type']==1) {$usertypename="普通用户";} else {$usertypename="华为用户";}
			$str.="\n".$usertypename."\t".$a['username']."\t".$a['type']."\t";
			$str.=$a['mobile']."\t";
			$str.=$a['email']."\t";
			$str.=$a['meiid']."\t";
			$str.=$a['purchase_time']."\t";
			$str.=$a['create_time']."\t";
			$str.=$a['appointment_from']."\t";
			$str.=$a['appointment_no'];
		} 
	
		$res = $this->ExcelOut($str);
	}

    public function ExcelOut($Str)
	{
...
//		echo $Str;
     ...
     * @brief 订单列表
     */
    public function order_appoint_list() {
        $data = array();
        ...
        $end_time = IFilter::act(IReq::get('end_time'));
        $where = '';
        if ($regtype != '') {$where .= ' and o.type = ' . $regtype;}   //注册类型 MeiID
        if ($usertype != '') {$where .= ' and c.user_type = ' . $usertype;}        //用户类型  普通，网盘
        if ($vf != '') {$where .= " and c.vf = {$vf}";}   //来源
		if ($status != '') {$where .= " and c.status = {$status}";}   //状态
		if ($name != '') {$where .= " and (o.mobile like '%{$name}%' or o.email like '%{$name}%') ";}   //用户名搜索
        if ($order_no != '') {$where .= " and c.appointment_no like '%{$order_no}%' ";}		//预约号搜索
		if ($start_time != '') {$where .= " and c.create_time>='{$start_time}' ";}	  //开始时间 
		if ($end_time != '') {$where .= " and c.create_time<='{$end_time}' ";}   //结束时间
        ...
        //向前台渲染
        $this->setRenderData($data);
        $this->redirect("order_appoint_list");
    }
     ...
     * @brief查看服务订单
     */
    public function order_service_show() {
        //获得post传来的值
        $order_id = IFilter::act(IReq::get('id'), 'int');
        $data = array();
        if ($order_id) {
            $order_show = new Order_Class();
            $data = $order_show->getOrderServiceShow($order_id);
            if ($data) {
                //获取地区
                $data['area_addr'] = join('&nbsp;', area::name($data['province'], $data['city'], $data['area']));
                ...
                $this->setRenderData($data);
                //var_dump($data);
                $this->redirect('order_service_show', false);
            }
        }
        if (!$data) {
            $this->redirect('order_service_list');
        }
    }
   ...
   /**
    ...
    public function appoint_show() {
        ...
        $data = array();
        if ($collection_id) {
            $tb_collection = new IQuery('appointment as c ');
            $tb_collection->join = ' left join user as o on c.mei_id=o.meiid';
            $tb_collection->fields = 'o.username,o.mobile,o.email,o.type,c.appointment_no,c.create_time,c.purchase_time,c.order_id,c.mei_id,c.vendor_id,c.pic_url,c.user_type,c.appointment_from';
            $tb_collection->where = 'c.order_id=' . $collection_id;
            $collection_info = $tb_collection->find();
            if ($collection_info) {
                $data = $collection_info[0];

                $this->setRenderData($data);
                $this->redirect('appoint_show', false);
            }
        }
        if (count($data) == 0) {
            $this->redirect('order_appoint_list');
    ...
    public function order_complete() {
        ...
        $r1 = $tb_order->update('id=' . $order_id);
    ...
    public function service_complete() {
        ...
        //oerder表的对象
        $tb_order = new IModel('order_service');
        $tb_order->setData(
                array(
                    'status' => 7,
    ...
    public function order_prepare() {
        ...
        $tb_order_log = new IModel('order_log');
        $action = '配货';
        $note = '订单【' . $order_no . '】配货成功';

        $logObj = new log('db');
        $logObj->write('operation', array("管理员:" . ISafe::get('admin_name'), "订单更新为配货", '订单号：' . $order_no));
     ...
     * @brief 服务单备货
     * */
    public function service_prepare() {
        //去掉左侧菜单和上部导航
        $this->layout = '';
        $order_id = IFilter::act(IReq::get('id'), 'int');
...
/**
     * @brief 服务单备货
     * */
    public function service_operate() {
        //去掉左侧菜单和上部导航
        $this->layout = '';
        $order_id = IFilter::act(IReq::get('id'), 'int');
...
/**
     * @brief 服务发货订单页面
     */
    public function service_deliver() {
        //去掉左侧菜单和上部导航
        $this->layout = '';
        $service_id = IFilter::act(IReq::get('id'), 'int');
        $data = array();
        if ($service_id) {
            $order_show = new Order_Class();
            $data = $order_show->getOrderServiceShow($service_id);
        }
        $this->setRenderData($data);
        $this->redirect('service_deliver');
    }
    ...
    public function order_delivery_doc() {
        ...
        die('<script type="text/javascript">parent.actionCallback("出库成功");</script>');
    }
    
    /**
     * @brief 发货操作
     */
    public function service_delivery_doc() {
        //获得post变量参数
        $order_id = IFilter::act(IReq::get('id'), 'int');
        Order_Class::sendDeliveryGoodsService($order_id,$this->admin['admin_id']);
    ...
    public function service_update(){
        ...
        $orderDB = new IModel("order");
        $order_data = $orderDB->getObj("order_no = '$order_no'");
        
        $order_id = $order_data["id"];
        $ogDB = new IModel("order_goods");
        $goods_data = $ogDB->getObj("order_id = $order_id");
        
        $db = new IModel("order_service");
        $data = array("order_id" => $order_id,
            "order_goods_id" => $goods_data["id"],
     ...
     * @brief 服务订单列表
     */
    public function order_service_list() {
        $data = array();

        $name = IFilter::act(IReq::get('name'));
        $service_no = IFilter::act(IReq::get('service_no'));
        $where = '';
        if ($name) {
            $where .= " and u.username like '%{$name}%'";
        }
        if ($service_no) {
            $where .= " and o.service_no like '%{$service_no}%'";
        }
        $data['name'] = $name;
     ...
     * @brief 修改服务订单
     */
    public function order_service_edit() {
        $data = array();

        //获得service_id的值
        $service_id = IFilter::act(IReq::get('id'), 'int');
        if ($service_id) {
            $serviceDB = new IModel('order_service');
            $data = $serviceDB->getObj('id = ' . $order_id);
            $this->orderRow = $data;
        ...
        $this->redirect('order_service_edit');
    }
     ...
     * @brief 一键订单关闭
     */
    public function order_kill() {
        //post数据
        $dead_time = IFilter::act(IReq::get('dead_time'), 'int');

        //生成order对象
        $tb_order = new IModel('order');
        $tb_order->setData(array('status' => 3));
        if (!empty($dead_time)) {
            $dead_time = time() - $dead_time*24*3600;
            //获取订单编号
            $orderRs = $tb_order->query(" status = 1 and pay_status=0 and  unix_timestamp(create_time) <= $dead_time", 'order_no');
            $orderData = array();
            foreach ($orderRs as $val) {
                $orderData[] = "'".$val['order_no']."'";
            }
            $tb_order->update(" order_no in (".join(',', $orderData).") ");

            $logObj = new log('db');
            $logObj->write('operation', array("管理员:" . ISafe::get('admin_name'), "订单超时取消", '订单号：' . join(',', $orderData)));

            $this->redirect('order_list');
        } else {
            $this->redirect('order_list', false);
            Util::showMessage('请选择订单超时的时限');
        }
    ...
    public function collection_del() {
	  ...
	  /**
     * @brief 预订单删除功能_删除到回收站
     */
    public function appoint_del() {
        //post数据
        $id = IFilter::act(IReq::get('id'), 'int');
		//生成order对象
        $tb_order = new IModel('appointment');
        $tb_order->setData(array('if_del' => 1));
        if ($id) {
            $tb_order->update(Order_Class::getWhere2($id));

            $logObj = new log('db');
            $logObj->write('operation', array("管理员:" . ISafe::get('admin_name'), "预订单移除到回收站内", '预定单ID：' . join(',', $id)));

            $this->redirect('order_appoint_list');
        } else {
            $this->redirect('order_appoint_list', false);
            Util::showMessage('请选择要删除的数据');
        }
    }
	...
	/**
     * @brief 预定单删除功能_删除回收站中的数据，彻底删除
     */
    public function appoint_recycle_del() {
        //post数据
        $id = IFilter::act(IReq::get('id'), 'int');
        //生成order对象
        $tb_order = new IModel('appointment');
        if ($id) {
            $tb_order->del(Order_Class::getWhere2($id));

            $logObj = new log('db');
            $logObj->write('operation', array("管理员:" . ISafe::get('admin_name'), "删除回收站内的预定单", '预定单ID：' . join(',', $id)));

            $this->redirect('appoint_recycle_list');
        } else {
            $this->redirect('appoint_recycle_list', false);
            Util::showMessage('请选择要删除的数据');
        }
    }
 ...
 /**
     * @brief 还原预定单列表
     */
    public function appoint_recycle_restore() {
        //post数据
        $id = IFilter::act(IReq::get('id'), 'int');
        //生成order对象
        $tb_order = new IModel('appointment');
        ...
        if ($id) {
            ...
            $logObj = new log('db');
            $logObj->write('operation', array("管理员:" . ISafe::get('admin_name'), "恢复了回收站内的预定单", '预定单ID：' . join(',', $id)));
    ...
    public function pick_template() {
        ...
        $data['deliver'] = current($deliver_doc_info);
        $this->setRenderData($data);
