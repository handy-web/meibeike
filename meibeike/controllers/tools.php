...
class Tools extends IController
{
	...
	{
		$id = intval(IReq::get("id"));
		$this->help_row=array('id'=>$id,'name'=>'','cat_id'=>9999,'content'=>"",'sort'=>0,'desp'=>'');
		if($id)
		{
			$this->help_row=SiteHelp::get_help_by_id($id);
			if(!isset($this->help_row[$id]))
			{
				$this->redirect("news_list",true);
				Util::showMessage( "没有这条记录" );
			}
			$this->help_row=$this->help_row[$id];
		}
	...
	/****常见问题开始******/
	function common_problem_list()
	{
		$query = new IQuery("help AS help");
		$query->where = " help.cat_id=9998";
		$query->fields = "help.*";
		$query->order = "help.`sort` ASC,help.id DESC";
		$query->page = isset($_GET['page'])?$_GET['page']:1;
		$this->query = $query;
		$this->list = $query->find();
		$this->redirect("common_problem_list");
	}
	
	function common_problem_edit()
	{
		$id = intval(IReq::get("id"));
		$this->help_row=array('id'=>$id,'name'=>'','cat_id'=>9998,'content'=>"",'sort'=>0,'desp'=>'');
		if($id)
		{
			$this->help_row=SiteHelp::get_help_by_id($id);
			if(!isset($this->help_row[$id]))
			{
				$this->redirect("common_problem_list",true);
				Util::showMessage( "没有这条记录" );
			}
			$this->help_row=$this->help_row[$id];
		}
		$this->redirect("common_problem_edit");
	}
	
	function common_problem_edit_act()
	{
		//数据在Sitemap里过滤了
		$data['id'] = IReq::get("id");
		$data['cat_id'] = 9998 ;
		...
		{
			$this->redirect("common_problem_edit",false);
			Util::showMessage($re['data']);
		}
	}
	
	function common_problem_del()
	{
		$id = IReq::get("id");
		if($id===null)
		{
			$this->redirect('/tools/common_problem_list');
		}
		$re = SiteHelp::help_del($id);
