...
get_delivery("广东");
		...
		<form action='<?php echo IUrl::creatUrl("/wap/cart_diy_3");?>' method='post' name='order_form' callback='orderFormInstance.isSubmit();'>
		<input type='hidden' name='timeKey' value='<?php echo time();?>' />
		<?php if($this->gid3){?><input type='hidden' name='direct_gid3' value='<?php echo isset($this->gid3)?$this->gid3:"";?>' /><?php }?>
		<?php if($this->gid7){?><input type='hidden' name='direct_gid7' value='<?php echo isset($this->gid7)?$this->gid7:"";?>' /><?php }?>
		<?php if($this->gid4){?><input type='hidden' name='direct_gid4' value='<?php echo isset($this->gid4)?$this->gid4:"";?>' /><?php }?>
		<?php if($this->gid3){?><input type='hidden' name='direct_num3' value='<?php echo isset($this->num3)?$this->num3:"";?>' /><?php }?>
		<?php if($this->gid7){?><input type='hidden' name='direct_num7' value='<?php echo isset($this->num7)?$this->num7:"";?>' /><?php }?>
		<?php if($this->gid4){?><input type='hidden' name='direct_num4' value='<?php echo isset($this->num4)?$this->num4:"";?>' /><?php }?>
		...
		<?php foreach($this->productList3 as $key => $item){?>
		...
		<?php }?>
				...
				<?php foreach($this->productList7 as $key => $item){?>
				<?php if(intval(IReq::get('num')) > $item['store_nums'] || intval(IReq::get('num')) < 0){?>
					<?php IError::show(403,'购买的商品数量不正确或者大于库存量');?>
				<?php }?>	
		<div class="cart-tab">					
		...
		<?php }?>
				...
				<?php foreach($this->productList4 as $key => $item){?>
				<?php if(intval(IReq::get('num')) > $item['store_nums'] || intval(IReq::get('num')) < 0){?>
					<?php IError::show(403,'购买的商品数量不正确或者大于库存量');?>
				<?php }?>	
		<div class="cart-tab">					
		...
		<div class="cart-list">
			<ul class="cart-ul">
				...
				<li>
					<textarea rows="3" cols="10" name='message' onblur="if(this.innerHTML==''){this.innerHTML='有特殊请求请留言（限30字）';this.style.color='#666'}" onfocus="if(this.innerHTML=='有特殊请求请留言（限30字）'){this.innerHTML='';this.style.color='#666'}">有特殊请求请留言（限30字）</textarea>
				</li>
			</ul>
		</div>
