...
//		var child = $(this).children(".menu_select_child");
...
//			var height2 = $(child).height();
//			$(this).height(height1 + height2);
//			$(this).css("box-shadow", "0 0 20px #000");
//			$(child).css("visibility", "visible");
//		}
	});
	
	$(".menu_select").mouseover(function() {
//		if ($("#show_select_menu").val() == 1)
//			return;
		...
		var height1 = $(this).height();
		var height2 = $(child).height();
		//$(this).css("height", height1 + height2);
		//$(this).height(height1 + height2);
		if ($("#all_menu_child").val() == 1)
			$(this).css("height", "223px");		// 这里写死，防止变化
		else
			$(this).css("height", "118px");
		$(this).css("box-shadow", "0 0 5px rgb(180, 180, 180)");
		$(child).css("visibility", "visible");
		
		$("#show_select_menu").val($(this).height());
	}).mouseout(function() {
		//$("#show_select_menu").val(0);
		
		var child = $(this).children(".menu_select_child");
		var height1 = $(this).height();
		var height2 = $(child).height();
		$(this).css("height", height1 - height2 - 5);
		//$(this).height(height1 - height2 - 5);
		$(this).css("box-shadow", " 0 0 0 rgb(234, 235, 239)");

		$(child).css("visibility", "hidden");
		$(this).css("background", "rgb(234, 235, 239)");
	});
});
