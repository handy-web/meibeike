...
function orderFormClass()
{
	...
	{	
		...
		//F码金额
		this.fcodePrice = $('input[name="fcode"]').attr('alt');
		//最终金额
		this.orderAmount = parseFloat(this.goodsSum) - parseFloat(this.fcodePrice) - parseFloat(this.ticketPrice) + parseFloat(this.deliveryPrice) + parseFloat(this.paymentPrice) + parseFloat(this.taxPrice) + parseFloat(this.protectPrice);
		//this.orderAmount = parseFloat(this.goodsSum) - parseFloat(this.fcodePrice);
		this.orderAmount = this.toDecimal2(this.orderAmount);
		
		$('#final_sum_first').text(this.orderAmount);
		//刷新DOM数据
	...
	{	
		...
		{		if($('input:radio[name="radio_address"][value!=""]').length > 0){					
	...
	{	
		//刷新修改form部分
		$('form[name="order_form"] #address_form input[type="text"]').each(function()
		{	
	...
	this.addressModSwith = function(action)
	{	
		//要切换的模式
		var toggleMod = this.addressMod == 'exit' ? 'edit' : 'exit';
		...
		if(action == 'add'){
			this.provinceMenuInit();
			$('.cover-m-1 input[type="text"]').each(function(){
				this.value = '';
			});
			
			$('.cover-m-1 input[type="hidden"]').each(function(){
				this.value = '';
			});
			$('.cover-title-1 span:eq(0)').html('新增地址');
			$('#address_add').show();
			$('#address_modify').hide();
		}else if(action == 'modify' ){
			$('.cover-title-1 span:eq(0)').html('修改地址');
			$('#address_modify').show();
			$('#address_add').hide();
		}
	...
	this.addressCheck = function()
	{
		...
		$('.cover-m-1').trigger('submit',function(){return false;});
		//数据格式不正确
		...
		if($('.cover-m-1 .invalid-text').length > 0){
			return false;
		}
